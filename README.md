...
6. **Docker 一键部署**
   ...
   docker-compose up -d
   ```
   注：**新建的产品必须要在权限管理哪里给对应登录用户授权分配产品，没有授权分配产品不显示编辑**
7. **演示地址**
   ```bash
...
9. **最佳使用方式案例**
- 当测试人员在测试环境进行版本部署时，需要先在系统平台将版本锁定，避免其他测试人员在测试过程中对该版本进行修改。
- 执行jenkins流水线发布环境时使用shell脚本version_lock.sh调用，实现版本的锁定与解锁。版本锁定，流水线退出，版本未锁定则流水线继续执行。
